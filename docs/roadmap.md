...
- [ ] ⚠️ **Resto de `npm audit`: 2 CRITICAL esperando decisión.** `next-auth` (beta.31) y `@auth/core` (0.41.2) arrastran cuatro advisories de Auth.js, entre ellas **«Configuration errors can cause existence-based auth checks to fail open»** y «el state, el nonce y el PKCE del OAuth no quedan atados al provider que los creó». El arreglo es un solo escalón: **beta.31 → beta.32** y `@auth/core` 0.41.3. No se hizo de oficio porque toca el camino de auth y este proyecto ya perdió una noche con Auth.js (las 305 `MissingCSRF` del 10/08), y un login real con Google no se puede probar localmente.
- [ ] `postcss` (4 high) y `sharp` (1 high) — cuelgan de `next`, y no del `postcss` de arriba: el que queda es el que Next trae empaquetado (`node_modules/next/node_modules/postcss@8.4.31`). npm sólo los da por resueltos con **`next@16.3.1`**, que sale de la línea 16.2.x. Las CVEs de `sharp` siguen en camino muerto por `images.unoptimized: true`.
